...
class Penerimaan extends Model {
	...
	public static function fetchHead($pener_id){
		$i = new static;
		return self::select($i->table.'.pener_date', $i->table.'.created_at', 'B.po_no', 'B.po_tgl_kedatangan', 'D.sup_nama')
					->join('po_laravel AS B', $i->table.'.po_id', '=', 'B.po_id')
					->join('permintaan_barang AS C', 'B.pb_id', '=', 'C.pb_id')
					->join('supplier_laravel AS D', 'C.sup_id', '=', 'D.sup_id')
	...
	public static function fetchKabagCheck($args){
		$i 		= new static;
		$get	= $i
			->select($i->table.'.'.$i->primaryKey, 'B.po_no', 'D.sup_nama', 'B.po_tgl_kedatangan', $i->table.'.pener_date', $i->table.'.kabag_check')
			->join('po_laravel AS B', $i->table.'.po_id', '=', 'B.po_id')
			->join('permintaan_barang AS C', 'B.pb_id', '=', 'C.pb_id')
			->join('supplier_laravel AS D', 'C.sup_id', '=', 'D.sup_id')
		...
		if(! is_null($args['search']['s'])){
			switch($args['search']['field']){
				case 'po_no' 		: $prefix = 'B.'; break;
				case 'sup_nama'		: $prefix = 'D.'; break;
				case 'pener_date'	: $prefix = $i->table.'.'; break;
			}
